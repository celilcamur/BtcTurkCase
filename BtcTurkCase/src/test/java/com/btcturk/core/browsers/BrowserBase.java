...
public abstract class BrowserBase {
    ...
    abstract RemoteWebDriver create();
    ...
    BrowserBase(DesiredCapabilities caps, String driverFileName) {
        this.caps = caps;
            ...
            setBrowserLogging(Level.WARNING);
    }

    private Platform getPlatform() {
        String osName = System.getProperty("os.name").toUpperCase();
        return osName.contains("WINDOWS") || osName.contains("WİNDOWS") ? Platform.WINDOWS
    ...
    private File getDriverFile(Platform platform, String fileName) {
        String driverPath = String.format(PATH_FORMAT, platform.toString(), fileName).toLowerCase().replace("ı", "i");
